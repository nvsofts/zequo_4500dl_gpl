...
/*YiChun patch from Marvell for MXIC bit flap issue 20190730 start*/
...
/*YiChun patch from Marvell for MXIC bit flap issue 20190730 end*/
...
static struct orion_nfc_naked_info orion_nfc_naked_info_lkup[NFC_PAGE_SIZE_MAX_CNT][MV_NFC_ECC_MAX_CNT] = {
	...
	{  {	/* Hamming */
		...
		&ecc_layout_2KB_bch4bit, &mv_lp_bb, 2048, 2048, 32, 1, 0, 0
                ...
                /*YiChun patch from Marvell for MXIC bit flap issue 20190730 start*/
                #if 0
		NULL, NULL, 2018, 1024, 0, 1, 1024, 32f
                #endif
...
static int orion_nfc_do_cmd_pio(struct orion_nfc_info *info)
{
	...
	for (i = 0; i < numCmds; i++) {
		...
		if (ndcr & NFC_CTRL_ND_RUN_MASK) {
			mvOsUDelay(10);//wait a while for device to be ready
