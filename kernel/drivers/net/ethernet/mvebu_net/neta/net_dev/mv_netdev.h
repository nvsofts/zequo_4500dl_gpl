...
struct eth_port {
	...
	MV_U32              sgmii_serdes;
	int	                pm_mode;
	/* port link parameters, used for comparing with phy link status in phy link polling */
	int                 link;
	int                 speed;
	int                 duplex;
	/*when port is not started, the link status change event should not be reported until the port is started*/
	int                 link_event_suppressed;
};
