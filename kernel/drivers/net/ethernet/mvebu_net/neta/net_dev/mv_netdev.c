...
/* TRUE: */
/* there are GBE MACs connected to out-of-band PHY, */
/* then the corresponding ports should use pp SMI with SW polling method, */
/* mv_eth_link_polling will be set true to indicate that the SW polling thread needs to be created;*/
/* FALSE: */
/* there are no GBE MACs connected to out-of-band PHY */
/* then all ports should use internal SMI by HW PHY polling, */
/* mv_eth_link_polling will be set false to indicate that the SW polling thread needn't to be created.*/
static bool mv_eth_link_polling;
/* the pp SMI SW polling thread to get out-of-band PHYs' status, speed and duplex*/
static struct task_struct *mv_eth_link_polling_thrd;
...
/*get indrect smi plat data from fdt file*/
static int mv_eth_indirect_smi_plat_data_get(struct device_node *mdio_node, struct mv_neta_pdata *plat_data)
{
	u32 smi_iobase_size[2];

	memset(smi_iobase_size, 0, sizeof(smi_iobase_size));

	/*read out Switch MG register base&offset&size and remap the address*/
	/*Switch MG reg format is <base size>*/
	if (of_property_read_u32_array(mdio_node, "reg", smi_iobase_size, 2)) {
		pr_err("could not PHY SMI address\n");
		return -ENODEV;
	}

	plat_data->oob_phy_smi_base = smi_iobase_size[0];
	plat_data->oob_phy_smi_remap_size = smi_iobase_size[1];

	return 0;
}

/*disable indirect smi MDC inversion*/
static void mv_eth_indirect_smi_mdc_inversion_disable(void __iomem *smi_virtual_addr)
{
	...
	if (of_machine_is_compatible("marvell,msys-bc2")) {
		...
		smi_conf = readl(smi_virtual_addr + pp_smi_misc_off);
		...
		pr_err("%s: this machine does not support indirect SMI access!\n", __func__);
		return;
	}

	writel(smi_conf, smi_virtual_addr + pp_smi_misc_off);

	return;
}

/*indrect smi initialization function*/
static int mv_eth_indirect_smi_init(struct mv_neta_pdata *plat_data)
{
	unsigned int compl_region_id;
	phys_addr_t smi_compl_addr;
	void __iomem *smi_virtual_addr;
	int ret;

	/* indirect access SMI of oob phy requires ADdr/Compl */
	ret = mv_addr_compl_request_region(plat_data->oob_phy_smi_base, &compl_region_id, &smi_compl_addr);
	if (ret) {
		pr_err("%s: Failed to request address completion region for SMI indirect access\n", __func__);
		return -ENODEV;
	}

	/* Remap PP SMI registers */
	smi_virtual_addr = ioremap(smi_compl_addr, plat_data->oob_phy_smi_remap_size);
	if (!smi_virtual_addr) {
		pr_err("%s: Unable to remap SMI register\n", __func__);
		return -ENOMEM;
	}

	mvEthPhySmiAddrSet((MV_U32)smi_virtual_addr);
	mv_eth_indirect_smi_mdc_inversion_disable(smi_virtual_addr);
...
static struct mv_neta_pdata *mv_plat_data_get(struct platform_device *pdev)
{
	...
	if ((pdev->id % 2) == 1) {
		...
		/* get previous sharing port's status */
		if (of_property_read_string(eth_previous, "status", &status) < 0) {
			pr_err("%s status is not found for %s!!!\n", eth_previous->name);
			return NULL;
		}

		/*i f previous sharing port's is not enabled, then we need to map the IO memory for it*/
	...
	if (mdio_node && of_find_property(mdio_node, "smi-indirect-access", NULL)) {
		...
		if (0 != mv_eth_indirect_smi_plat_data_get(mdio_node, plat_data)) {
...
#ifdef CONFIG_OF
	/* for non-oob-phy port, init SMI register*/
	if (0 == plat_data->is_oob_phy)
...
/* This function will bring up the pp SMI SW polling thread if there are GBE MACs connected to out-of-band PHY*/
static __init int mv_eth_start_link_polling_thread(void)
{
	struct task_struct *thrd;
	struct sched_param param;

	/* Start ethernet link status polling thread if there are GBE MACs connected to out-of-band PHY*/
	if (mv_eth_link_polling) {
		thrd = kthread_run(mv_eth_link_polling_thread, NULL, "phy link polling");
		if (IS_ERR(thrd))
			pr_err("failed to start config thread\n");
		/* Set scheduler and priority */
		param.sched_priority = 99;
		sched_setscheduler(thrd, SCHED_FIFO, &param);
		pr_info("create eth link polling thread (pid=%d)\n", thrd->pid);
		mv_eth_link_polling_thrd = thrd;
	}

	return 0;
}

late_initcall(mv_eth_start_link_polling_thread);
