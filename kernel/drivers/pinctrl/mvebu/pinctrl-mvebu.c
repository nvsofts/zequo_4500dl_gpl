...
#include <../arch/arm/mach-mvebu/mts-arch.h>
				...
				unsigned gid, unsigned long config)
{
	...
	/* handle MTS MPP , if function return with 0 means MPP overwrite by MTS else need to continue configure as planed*/
	if ( !mts_handle_mpp(pctl->base, grp->gid) )
	{
		/* MTS overwrite pin configuration*/
		return 0;
	}
